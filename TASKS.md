...
- [x] **9.1** Users page + `InviteForm` — HQ-admin-only (redirect-gated), profiles list (name/role/country/added) + invite panel. — `users/page.tsx`, `users/InviteForm.tsx`. ⚠️ **Superseded by 14.3:** `InviteForm` deleted, redirect gate removed, Role/Country columns dropped. **Extended 2026-08-05:** the list gained an actions column with a **Delete** button (shared `DeleteButton`) backed by new `DELETE /api/users/[id]` — deletes the **auth** user so the `profiles` FK cascade removes the profile (deleting the profile alone would strand an auth user on `/no-access`). Signed-in check only, same as `POST`; **self-deletion returns 400**, which is what guarantees the last account cannot be deleted from inside the app, and the caller's own row shows a "You" chip instead of the button. BUS-2 audit write + `createUserLimiter` carried over. Authored rows and `audit_log` entries survive — no FK on `created_by`/`actor`. ⚠️ Not driven live (12.4.2 / 12.4.6).
  **Extended again 2026-08-05:** the actions column also carries **Edit** (every row, own account included — unlike Delete there is nothing unsafe about editing yourself), backed by new `PATCH /api/users/[id]` and a `/users/[id]/edit` page. Three fields: `full_name` (in `profiles`), plus email and password (in `auth.users`, so the service-role client again). **A blank password box keeps the current password** — `updateUserSchema` normalises `""` → `undefined`; email changes use `email_confirm: true` so no SMTP is needed. Signed-in check only, `createUserLimiter` keyed `update-user:<uid>`, BUS-2 audit write whose diff records from→to for name/email and a bare `password_reset: true` — 🔴 never the password itself. ⚠️ **No optimistic concurrency**: `profiles` has no `updated_at`, so concurrent edits are last-write-wins. ⚠️ Not driven live (12.4.2 / 12.4.6).
...
- [ ] **12.4.2** **`POST /api/users`, `PATCH /api/users/[id]` and `DELETE /api/users/[id]` specifically:** 403 without a session, and the `createUserLimiter` throttle (10/min, shared by all three) actually holds under a burst. 🔴 **`PATCH` can set another account's password**, so treat it as an auth-table mutation, not an edit form. Every signed-in user can mint **and remove** accounts now, so this throttle is the only bound on the auth table. 🔴 Also probe the **self-delete refusal by hand** (not via the UI, which hides the button): `DELETE /api/users/<your-own-id>` must return **400**. That guard is the only thing preventing the last account from being deleted — public sign-up is disabled, so an empty `profiles` table locks everyone out permanently.
...
- [ ] **12.4.6** **Fold in the 14.6 residual** — click it through in a browser on staging: a second user signs in, sees all four countries, and creates a third from the Users page. Everything below the UI is proven; this path is not. **Now also covers delete (2026-08-05):** remove that third user from the Users page, confirm they can no longer sign in, and re-check **0 orphans in both directions** (no auth user without a profile, no profile without an auth user). **And edit (2026-08-05):** before deleting them, rename that user, change their sign-in email and confirm they sign in with the new address, then reset their password from the Edit page and confirm the new one works — plus that saving with the password box **blank** leaves the old password working.
