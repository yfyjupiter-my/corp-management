...
- [x] **11.1** RLS integration tests with two seeded users (1 `hq_admin`, 1 `country_manager` MY) asserting cross-country returns empty/denied (`tests/rls.test.ts`). **Run live 2026-07-28 — 4/4 passed.** The two users now exist on the linked project (`rls-test-hq@` / `rls-test-my@corp-management.test`) with credentials in the git-ignored `.env.test`.
