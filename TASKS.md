...
- [ ] **12.4.2** **`POST /api/users` and `DELETE /api/users/[id]` specifically:** 403 without a session, and the `createUserLimiter` throttle (10/min, shared by both) actually holds under a burst. Every signed-in user can mint **and remove** accounts now, so this throttle is the only bound on the auth table. 🔴 Also probe the **self-delete refusal by hand** (not via the UI, which hides the button): `DELETE /api/users/<your-own-id>` must return **400**. That guard is the only thing preventing the last account from being deleted — public sign-up is disabled, so an empty `profiles` table locks everyone out permanently.
...
- [ ] **12.4.6** **Fold in the 14.6 residual** — click it through in a browser on staging: a second user signs in, sees all four countries, and creates a third from the Users page. Everything below the UI is proven; this path is not. **Now also covers delete (2026-08-05):** remove that third user from the Users page, confirm they can no longer sign in, and re-check **0 orphans in both directions** (no auth user without a profile, no profile without an auth user).
