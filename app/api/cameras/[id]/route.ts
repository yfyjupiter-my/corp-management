...
/**
 * Hard-delete a single CCTV camera. RLS (via recorder → site) decides which rows
 * the caller may touch, and the delete is filtered by id alone — a row outside
 * the caller's country matches nothing, so a cross-country probe is
 * indistinguishable from a missing row (both 404). A camera is a leaf: nothing
 * cascades. `0003_audit.sql` records the deleted values.
 */
export async function DELETE(
  ...
  try {
    ...
    // SEC-5: cap writes per user (shares the write budget, namespaced by action).
    const rl = writeLimiter.check(`delete:cctv_cameras:${user.id}`);
    if (!rl.ok) return rateLimitResponse(rl, t);
    ...
    const { data: deleted, error } = await supabase
      ...
      .eq("id", id)
      .select("id");
    if (error) return dbErrorResponse(error, `DELETE /cameras/${id}`, t);
    ...
    console.error("[route-error] DELETE /cameras/[id]:", err);
    return NextResponse.json({ error: t.errors.serverError }, { status: 500 });
  }
}
